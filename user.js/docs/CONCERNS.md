...
|----------|---------|--------|-------------------|
| Medium | CI now runs `build_userscripts.py --check` + `node --check` + `node --test` on every push/PR (`.github/workflows/verify.yml`), but the `node:test` suite itself covers a meaningful minority of the codebase (title-setting, DP org-link-shortcut, the shared cache, CP's name-normalization/IP-CIDR/renumber/IX-F-merge-gate logic, retry/backoff, DP's whitelist-URL parsing — see [AGENTS.md](../AGENTS.md) Testing) | A regression in an untested module (most of CP's ~207 helper functions, most of FP/DP's DOM-touching modules) can still ship unnoticed until an admin hits it live | Continuing to expand: CP's IP/CIDR math, renumber classification, and IX-F merge/conflict-resolve 8-gate safety check are now covered (the highest consequence-of-failure surface in the repo — a false-positive gate pass leads directly to an operator deleting a live netixlan row). Remaining high-value targets: CP's name-pattern-diagnostics/audit-merge helpers, DP's linkify engine, FP's admin-ops URL builders |
