...
|----------|---------|--------|-------------------|
| Medium | CI now runs `build_userscripts.py --check` + `node --check` + `node --test` on every push/PR (`.github/workflows/verify.yml`), but the `node:test` suite itself covers a meaningful minority of the codebase (title-setting, DP org-link-shortcut/linkify-engine/renumber-launcher/CP-fallback/mailto, the shared cache, CP's name-normalization/IP-CIDR/renumber/IX-F-merge-gate/name-pattern-diagnostics logic, retry/backoff, DP's whitelist-URL parsing — see [AGENTS.md](../AGENTS.md) Testing) | A regression in an untested module (most of CP's ~207 helper functions, most of FP/DP's DOM-touching modules) can still ship unnoticed until an admin hits it live | Continuing to expand: CP's IP/CIDR math, renumber classification, IX-F merge/conflict-resolve 8-gate safety check (the highest consequence-of-failure surface in the repo), Network Name Pattern Diagnostics/Recent IP Changes audit reconciliation, and DP's linkify engine/renumber-launcher/CP-fallback-on-404/mailto helpers are now covered. Remaining high-value target: FP's admin-ops URL/report builders |
