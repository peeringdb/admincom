...
|----------|---------|--------|-------------------|
| Low | CI now runs `build_userscripts.py --check` + `node --check` + `node --test` on every push/PR (`.github/workflows/verify.yml`), and the `node:test` suite now covers every pure-logic target identified in the initial coverage-expansion pass (title-setting, DP org-link-shortcut/linkify-engine/renumber-launcher/CP-fallback/mailto, the shared cache, CP's name-normalization/IP-CIDR/renumber/IX-F-merge-gate/name-pattern-diagnostics logic, retry/backoff, DP's whitelist-URL parsing, FP's admin-ops URL/report builders — see [AGENTS.md](../AGENTS.md) Testing) | A regression in an untested module (most of CP's ~207 helper functions, most of FP/DP's DOM-touching modules) can still ship unnoticed until an admin hits it live | Downgraded from Medium: the highest-risk pure-logic surfaces across all three scripts are now covered. Remaining gaps are DOM-heavy modules already exercised by manual smoke testing, plus a handful of shared cache-helper edge cases (`getSharedCacheStorageKey` validation branch, `isNegativeCacheEntry`, `cacheNegativeLookup`) currently only exercised indirectly via `shared-cache.test.js` |
