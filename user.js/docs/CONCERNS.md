...
|----------|---------|--------|-------------------|
| Low | CI now runs `build_userscripts.py --check` + `node --check` + `node --test` on every push/PR (`.github/workflows/verify.yml`), and the `node:test` suite covers every pure-logic target identified across a 5-phase coverage-expansion pass (title-setting, DP org-link-shortcut/linkify-engine/renumber-launcher/CP-fallback/mailto, the shared cache including its negative-cache helpers, CP's name-normalization/IP-CIDR/renumber/IX-F-merge-gate/name-pattern-diagnostics logic, retry/backoff, DP's whitelist-URL parsing, FP's admin-ops URL/report builders — see [AGENTS.md](../AGENTS.md) Testing) | A regression in an untested module (most of CP's ~207 helper functions, most of FP/DP's DOM-touching modules) can still ship unnoticed until an admin hits it live | **"Complete" was wrong, and the guidance actively discouraged fixing it.** This row previously claimed every high-risk pure-logic target was covered and told readers to expand coverage only opportunistically. In fact `lib/admincom-common.js`'s retry wrappers had zero coverage, and the test shim could not have exercised them anyway — its fake response exposed no `headers.get()`, so the `Retry-After` path threw `TypeError` on contact. That is how a policy of retrying `PUT`/`DELETE` on any 429/5xx shipped unnoticed. Treat "covered" as a claim to re-verify against the harness, not a milestone. Coverage since: the retry wrappers (method-awareness included), the shim itself, CP's `pdbFetch` error contract and IX-F merge apply loop, FP's IX-F readable-vs-absent distinction, and the build script's include-marker guard. Genuinely remaining: DOM-heavy modules, which the shim's `FakeElement` cannot drive at all (no `addEventListener`) — those rely on the manual smoke test, and CP's modal safeguards are pinned by source assertions in `tests/cp-modal-safeguards.test.js` as a stopgap |
...
|------|-----------------|----------|---------------------|-----|
| `@connect *` wildcard cross-origin grant on CP and FP | A05 (Security Misconfiguration) | `peeringdb-cp-consolidated-tools.meta.js` `@connect *`; `peeringdb-fp-consolidated-tools.meta.js` `@connect *` (added with the `netixlan-ixf-verify` module) | Justified in-file: IX-F member-export hosts are operator-defined and can't be enumerated in advance. The IX-F fetches themselves are `anonymous: true` GET-only on both scripts (`peeringdb-cp-consolidated-tools.src.js:2939`, `peeringdb-fp-consolidated-tools.src.js:1099`). Destructive writes are same-origin `fetch`, and since the safe-method retry fix they are no longer replayed automatically — a transient 429/5xx fails the write and the admin retries by hand, rather than the script silently re-issuing a `DELETE` that may already have committed | **The GET-only claim covers the IX-F path only, and this row used to state it unqualified.** `pdbFetch` and `pdbPost` both fall back to `gmRequestWithRetry` with `withCredentials: true, anonymous: false` when a URL is not same-origin (`:6143`, `:6300`), and `pdbPost`'s fallback carries whatever method it was given, `PUT` and `DELETE` included. Under `@connect *` that is a credentialed arbitrary-method cross-origin capability, not a read-only one. No current call site sends a write to a non-PeeringDB origin, so this is latent rather than live — but it is a wider surface than the wildcard's justification describes. No allowlist restriction is possible while supporting arbitrary IX portals; scoping the *credentialed* fallback to the PeeringDB origins would be, and is not done |
