...
(function () {
  ...
  // >>> GENERATED by user.js/scripts/build_userscripts.py from peeringdb-deskpro-tools.src.js + lib/admincom-common.js <<<
  ...
  // >>> GENERATED by user.js/scripts/build_userscripts.py from peeringdb-deskpro-tools.src.js + lib/admincom-entity-exclusions.js <<<
  // >>> Do not hand-edit the block below; edit the source and regenerate. <<<
  ...
  // >>> GENERATED by user.js/scripts/build_userscripts.py from peeringdb-deskpro-tools.src.js + lib/admincom-shared-helpers.js <<<
  // >>> Do not hand-edit the block below; edit the source and regenerate. <<<
   ...
   * Retry/backoff/timeout policy (including Retry-After handling) is owned by
   * the shared fetchWithRetry/gmRequestWithRetry helpers from
   * admincom-common.js; `retries` is accepted for call-signature
   * compatibility but attempt count is governed by the shared MAX_RETRIES
   * policy, not this local value.
  ...
  async function pdbFetch(url, { headers = {}, timeout = PDB_API_TIMEOUT_MS, retries = PDB_API_RETRIES } = {}) {
    void retries; // Attempt count now governed by the shared MAX_RETRIES policy.
   ...
   * Proactive rate-limit backoff (shouldBackoffRateLimit, DP-specific: reads
   * x-ratelimit-* response headers to pre-emptively slow down before quota
   * is exhausted) runs first; the actual request/retry/timeout/backoff is
   * then delegated to the shared fetchWithRetry helper, which reactively
   * retries 429/5xx responses honoring any Retry-After header. `retries` is
   * accepted for call-signature compatibility but attempt count is governed
   * by the shared MAX_RETRIES policy, not this local value.
  ...
  /**
   * Registers DP Tampermonkey runtime toggles for debug mode and feature flags.
  ...
  function registerDpMenuCommands() {
    ...
    let debugToggleId = null;
    let flagShowId = null;
    let flagResetId = null;
    let flagToggleIds = [];

    /**
     * Builds the self-describing label for the debug-mode toggle command.
     * @ai Preserve menu command registration behavior.
     * @returns {string} Label reflecting current debug state and the action.
     */
    const getDebugToggleMenuLabel = () => {
      const enabled = isDebugEnabled();
      return `DP: Debug Mode [${enabled ? "ON" : "OFF"}] (toggle to ${enabled ? "OFF" : "ON"})`;
    };

    const registerCommands = () => {
      // Unregister existing commands
      if (typeof GM_unregisterMenuCommand === "function") {
        if (debugToggleId != null) GM_unregisterMenuCommand(debugToggleId);
      ...
      // Toggle debug mode (shared with CP/FP via admincom-common.js toggleDebugMode()).
      debugToggleId = GM_registerMenuCommand(getDebugToggleMenuLabel(), () => {
        const next = toggleDebugMode();
        console.info(`[DP] Debug mode ${next ? "enabled" : "disabled"}.`);
        registerCommands();
      });
