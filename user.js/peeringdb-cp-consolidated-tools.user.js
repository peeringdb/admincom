...
(function () {
  ...
  // DIAGNOSTICS_STORAGE_KEY is defined by the admincom-common.js include below.
  ...
  // >>> GENERATED by user.js/scripts/build_userscripts.py from peeringdb-cp-consolidated-tools.src.js + lib/admincom-entity-exclusions.js <<<
  ...
  // Shared diagnostics/debug-logging and request-retry helpers (DIAGNOSTICS_STORAGE_KEY,
  // isDebugEnabled, toggleDebugMode, dbg/dbgInfo/dbgWarn/dbgGroup/dbgGroupEnd,
  // REQUEST_TIMEOUT_MS, RETRYABLE_STATUS, MAX_RETRIES, parseRetryAfterMs,
  // classifyRetry, gmRequestWithRetry, fetchWithRetry) are inlined from
  // user.js/lib/admincom-common.js at build time; see scripts/build_userscripts.py.
  // Relies on MODULE_PREFIX and isFeatureEnabled(), both already defined above.
  // >>> GENERATED by user.js/scripts/build_userscripts.py from peeringdb-cp-consolidated-tools.src.js + lib/admincom-common.js <<<
  // >>> Do not hand-edit the block below; edit the source and regenerate. <<<
  ...
  // isDebugEnabled()/dbg() come from the admincom-common.js include above
  // (Tampermonkey menu label still references "CP: Toggle Debug Mode").
  ...
  // Org/RDAP name-normalization helpers (stripCompanyTypeSuffix,
  // sanitizeRdapOrgName, parseOrganizationNameIdentity, etc.) are inlined from
  // user.js/lib/cp-name-normalization.js at build time; see
  // scripts/build_userscripts.py. Pure functions, no dependency on anything
  // defined above.
  // >>> GENERATED by user.js/scripts/build_userscripts.py from peeringdb-cp-consolidated-tools.src.js + lib/cp-name-normalization.js <<<
  // >>> Do not hand-edit the block below; edit the source and regenerate. <<<
  ...
  * @returns {string} Human-readable label (e.g., "IX (FP)").
  ...
  async function pdbFetch(url, { headers = {}, timeout = PDB_API_TIMEOUT_MS, retries = PDB_API_RETRIES } = {}) {
    ...
    // retries > 1 opts into the shared wrapper's own backoff/Retry-After retry
    // loop (capped at the lib's MAX_RETRIES); retries <= 1 -- the default, and
    // what every current call site passes -- makes exactly one attempt, same
    // as the previous hand-rolled loop.
    const allowRetry = retries > 1;
  ...
  * @returns {Promise<{ status: number, data?: object|null, rawBody?: string, authStatus?: string, headersRaw?: string, reason?: string, csrfSent?: boolean }>} Response metadata.
  ...
  async function pdbPost(url, method = "POST", body = "", { headers = {}, contentType = "application/json", timeout = PDB_API_TIMEOUT_MS, retries = PDB_API_RETRIES } = {}) {
    ...
    try { requestOrigin = new URL(url, window.location.origin).origin; } catch (_err) { /* keep empty origin */ }
    const isSameOrigin = requestOrigin === window.location.origin;
    // See pdbFetch above: retries > 1 opts into the shared wrapper's backoff/
    // Retry-After retry loop; retries <= 1 (every current call site) makes
    // exactly one attempt, same as the previous hand-rolled loop/recursion.
    const allowRetry = retries > 1;
  ...
  * Necessity: Prevents stale retries from affecting later manual edits.
  ...
  const rdapAutnumClient = (() => {
    ...
    async function resolveOrganizationNameByAsn(asnInput) {
      ...
      dbg("rdap", `Requesting RDAP for AS${asn}...`);
      ...
      try {
        ...
        if (name) {
          dbg("rdap", `Successfully resolved AS${asn}: ${name}`);
        ...
        dbgWarn("rdap", `Error resolving AS${asn}`, _error);
        return null;
      }
    }
